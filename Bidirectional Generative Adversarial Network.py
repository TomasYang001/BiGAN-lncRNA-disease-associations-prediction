...
def related_association_finding(i): # i is the position of labeled feature
    ...
    Testing_set = Testing_set_1.reshape(-1, 6137)
    for num in range(0, 2282426):
        if unlabel_position[0][num] == j:
            # list_num = list_num.append(num)
            x = Unlabeled[num].reshape(-1, 6137)
...
def Score_function(Testing_one):
    ...
    b = tf.reshape(gen, [Testing_one.shape[0], 6137])
    distance = tf.square(b - Testing_one)
    ones = tf.ones([6137, 1], dtype=tf.float32)
...
if __name__ == '__main__':
    for num in range(0,num_folds):# 0 - num_folds
        start = int( num * 7696 // num_folds)
        end = int( ( num + 1 ) * 7696 //num_folds )
        #Testing_set = X_train[start:end] #(1086, 6137)
        Training_set = np.delete(X_train, np.arange(1539) + start, axis = 0) # (4344, 6137)
        bigan = BIGAN(Training_set)
        bigan.train(epochs = 45700, batch_size=128)
    ...
    label = np.concatenate((np.ones(7696),np.zeros(7696)),axis = 0)
